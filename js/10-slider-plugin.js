    ...
    return this.each(function () {
        ...
        if (!sliderObj.clicked) {
          ...
          if (sliderOptions.loop) {
            if (sliderObj.move == lastSlide) {
              $this.find(".slider-banner").css("transition", "none");
              sliderObj.move = 0;
            } else {
              sliderObj.move = sliderObj.move + 1;
              $this.find(".slider-banner").css("transition", "transform 0.5s, margin-left 0.5s ease-out, -webkit-transform 0.5s");
            }
          } else {
      ...
      function prevFn() {
        if (!sliderObj.clicked) {
          ...
          if (sliderOptions.loop) {
            if (sliderObj.move <= 0) {
              $this.find(".slider-banner").css("transition", "none");
              sliderObj.move = sliderObj.itemsLength - 1;
            } else {
              sliderObj.move = sliderObj.move - 1;
              $this.find(".slider-banner").css("transition", "transform 0.5s, margin-left 0.5s ease-out, -webkit-transform 0.5s");
            }
      ...
      function mouseDrag() {
        ...
        $(document).on("mousemove", function (e) {
          e.preventDefault();
          if (sliderObj.mousedown) {
            // console.log(e.pageX - sliderObj.pageX);
